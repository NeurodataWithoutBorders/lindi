...
# Benchmark writing a large number of small and large datasets to .lindi.tar,
# .zarr, .h5, and .dat files
...
def benchmark_h5py(
    ...
    if mode == "dat":
        ...
        for i in range(num_large_datasets):
            data = create_dataset(large_size)
            root.create_dataset(
                f"large_dataset_{i}", data=data, chunks=chunks, compressor=compressor
            )
    else:
        if mode == "h5":
        ...
        for i in range(num_large_datasets):
            data = create_dataset(large_size)
            ds = f.create_dataset(
                f"large_dataset_{i}", data=data, chunks=chunks, compression=compression
            )
            ds.attrs["attr1"] = 1
...
if __name__ == "__main__":
    ...
    lindi_time, total_size = benchmark_h5py(
        file_path_lindi,
        num_small_datasets,
        num_large_datasets,
        small_size,
    ...
    h5py_time, total_size = benchmark_h5py(
        file_path_h5,
        num_small_datasets,
        num_large_datasets,
        small_size,
